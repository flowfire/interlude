...
- **变了** → 照常输出完整场景（下面所有字段）。
- **没变**（还是那个地方、那段时间、那些人、那种气氛）→ 只输出
  {"unchanged": true}，**别的什么都不要写**。
  已经写过的环境不必再写一遍 —— 引擎会沿用上一轮那份。

**"没变"不等于"没进展"。** 剧情完全可以往前推进而场景照旧 ——
两个人还在那张桌边，只是话说到了一半；架还在打，只是换了个姿势。
**连续几轮待在同一处的对话，绝大多数都属于"没变"。**

注意：**第一轮没有上一轮**，所以第一轮必须输出完整场景。
时间明确跳了（"三天后"）、人走了或来了、换了地方 —— 这些都要输出完整场景。
