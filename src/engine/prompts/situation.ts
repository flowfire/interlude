...
【快速入戏 —— 用户要的是直接进入，不是铺垫】
用户勾了这个，意思很明确：**别铺了，这一轮就往性上走。**

- pace 直接给 escalate 或 climax，不要再用 build 消耗这一轮。
- act 要写**明确的性进展**，不是"气氛暧昧了一点"：谁脱了谁的衣服、
  谁把谁按在哪、手伸进了哪里、口交还是插入、谁在上面 ——
  具体到**发生了什么、身体在哪、谁对谁做了什么**。
- events 也可以直接为这件事服务：门锁上了、外面不会有人来、最后一点灯灭了、
  雨声盖住动静。
...
- 如果这一轮的关系还没到那一步，**给它一个说得通的由头**让它能到：
  酒精、久别、劫后余生、一次摊牌之后的失控。别用"他们忽然就想通了"。
