...
　**每个角色都会经过同一层「信息分发」。** 一轮跑到这里，引擎会调用一次模型，把「这一轮实际发生了什么」转化成**每个角色各自接收到的版本**——谁背对着所以没看见、谁在走神所以只听见半句、谁感官超常所以连你藏在背后的手都察觉到了。扮演环节只拿到自己那一份，原文根本不会进入它的上下文。

　**什么都没多察觉到是常见且正确的答案**，提示词里写死了这一条——否则模型会把每条信息都分配出去。
