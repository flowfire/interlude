...
　**超常感官和读心走的是同一套机制。** 如果这个人的人设里写了「眼观六路、耳听八方」这类感官，或者读心类能力，引擎会先跑一个独立的 **「感知判定」** 步骤——由一次单独的调用判断他实际察觉到了什么（**可能什么都没多察觉到，这是常见且正确的答案**），扮演环节只拿到判定结果，原文根本不会进入他的上下文。

　明面上的东西（当面说的话、当着他面做的大动作）不走这一步，直接给他。所以普通角色不会多花这一次调用。
