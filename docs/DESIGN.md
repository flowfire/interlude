...
| 位置 | 内容 | 谁共享 |
|---|---|---|
| `system` | 通用指令 + 自由度 + 分级 | 同一轮的所有角色（实际上同一个项目里跨轮也一致） |
| `user` 前半段 | 共享背景：故事名、时间地点、pc 概况、在场名单、前文回顾 | 同一轮的所有角色 |
| 分隔线 | `━━━ 以下只属于「你」━━━` | —— |
| `user` 后半段 | 角色卡、能力、感知、标志性特征、语句示例、原作锚点、底线、记忆、眼前环境、自己的心理、额外察觉、实际接收、确定不知道的事、任务 | 只有这个角色自己 |
...
新增 `tests/cache.spec.ts` 六条断言，把这个约定钉死：
