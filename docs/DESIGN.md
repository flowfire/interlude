...
### 第二十四轮变更：「快速进入」+ 成人向下放宽「注意到」的门槛

**两个要求**（用户提出）：

1. 在勾了 R18 的基础上，再加一个**「快速进入」**：导演更倾向于快速进入剧情，
   旁白的用词更直白、直接，包括「鸡巴」「操」这类粗俗说法。
2. 成人向模式下，**用户的行为细节应该更容易被派发出去、更容易被注意到**。

**① 「快速进入」是叠在成人向上的一层，不是第三个分级**
...
（`isRoundDraftDirty` 认这一项），所以"只加勾一个快速进入"也能重新生成那一轮。
...
**新增测试** `tests/rating.spec.ts` 四条：快速进入两条都齐了才生效（只勾 R18 不出现、
勾了 R18 没勾它也不出现、没勾 R18 更不出现）、导演侧的节奏与用词、
信息分发侧的放宽与空间关系不受影响、以及"只加勾快速进入也算改动"。
